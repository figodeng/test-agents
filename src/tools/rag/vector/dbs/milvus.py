...
from backend.tools.rag.vector.main import (
    VectorDBBase,
    VectorItem,
    SearchResult,
    GetResult,
...
class MilvusClient(VectorDBBase):
    def __init__(self):
        self.collection_prefix = "aim"

        USE_LOCAL_MILVUS = os.getenv("USE_LOCAL_MILVUS").lower() == "true"
        MILVUS_URL = os.getenv("MILVUS_URL")
        MILVUS_DB = os.getenv("MILVUS_DB")
        if not USE_LOCAL_MILVUS:
            self.client = Client(uri=MILVUS_URL, db_name=MILVUS_DB)
        else:
            DATA_PATH = os.getenv("MILVUS_DB_FILE")
            if os.path.exists(DATA_PATH):
                log.info("milvus db:", DATA_PATH)
            else:
                log.info("文件不存在", DATA_PATH)
            self.client = Client(DATA_PATH)
